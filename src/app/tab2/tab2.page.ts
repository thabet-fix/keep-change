...
export class Tab2Page {
  ...
  ngOnInit() {
    ...
    this.merchantService
      .getMerchant(this.afAuth.auth.currentUser.uid)
      .subscribe((data) => {
  ...
  receiveMoney() {
    ...
    */
    ...
    this.merchantService.getMerchant(this.idMerchant).subscribe((data) => {
    ...
    let x = 0;
    this.clientService.getClient("u48o31mxPhFuuPH8LNbI").subscribe(
      //id Current Client
      (dataClient) => {
        ...
        if (this.moneyToReceive && this.moneyToReceive !== 0) {
          this.clientService
            .updateClient(this.actualClient, {
              moneyToReceive: this.moneyToReceive,
            })
            .then(() => {
              this.actualClient.moneyToReceive = this.moneyToReceive;
              this.moneyToReceive = 0;
              console.log(
                "Le client à reçu l'argent, send notification to client"
              );
  ...
  updateMerchant() {
    if (this.moneyToReceive == 0) {
      this.merchantService
        .updateMerchant(this.actualMerchant, { moneyToSend: 0 })
        .then(() => {
          this.historyService.addHistory(
            "merchant",
          ...
          console.log(
            "Le client à reçu l'argent, send notification to merchant"
          );
